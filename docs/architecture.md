...
| 事件循环      | crossterm event                  | 当前为同步事件循环；动画 tick 后续接入                                                          |
| 动画          | tachyonfx                        | 后续用于场景切换过渡特效（淡入淡出、径向扩散、颜色偏移等）                                      |
| Markdown 渲染 | ratatui Paragraph                | 当前按纯文本段落渲染；`tui-markdown` / pulldown-cmark 渲染为后续增强                             |
| 树状图        | ratatui 基础列表                 | 当前 `map` 使用 checkpoint 列表；`tui-tree-widget` 章节树展示为后续增强                           |
| 输入组件      | 自研轻量输入框                   | 当前支持中文宽度光标、基础编辑、菜单选择；自动补全后续接入                                      |
...
│       └── src/
│           ├── lib.rs              # TUI 入口
│           ├── app.rs              # Session 驱动、输入分发、Outcome 消费
│           ├── view.rs             # 布局与 widget 渲染
│           ├── input.rs            # 命令输入框
...
- 第三方 ratatui 生态 crate（tachyonfx、tui-markdown、tui-tree-widget、tui-prompts）暂未接入。后续接入前需确认它们对同一 ratatui 主版本的兼容性；**`tui-markdown` 相对小众，需先做 POC 验证**——确认其维护状态、渲染能力（中文排版、嵌套列表、代码块）是否满足需求。若不满足则继续基于 ratatui `Text`/`Paragraph` 自行渲染基础 Markdown
- `include_dir` 内嵌模式尚未实现；实现后，内嵌的 `data/` 目录变更需要重新编译
- 当前 crossterm 事件读取使用同步轮询；动画与外部异步任务接入后，再拆到独立线程或 async task，避免阻塞渲染
...
- 当前 `play` 会先校验内容，校验通过后进入 `darkbluff-tui` 基础渲染循环
