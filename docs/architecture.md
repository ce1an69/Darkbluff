...
| 事件循环      | crossterm event                  | 当前为同步事件循环；无动画时按输入/resize 重绘，动画期间使用短 tick 重绘                         |
| 动画          | TUI 内建轻量过渡                 | 当前用于 gaze / move / chapter / narrative / ending 的短提示动画；复杂特效库后续再评估          |
| Markdown 渲染 | ratatui Paragraph                | 当前实现轻量 Markdown 子集渲染（标题、列表、段落）；复杂 Markdown 渲染为后续增强                 |
| 树状图        | ratatui 基础列表                 | 当前 `map` 面板展示已探索章节、可见未知分支、checkpoint 与话题进度；第三方树控件后续再评估       |
| 输入组件      | 自研轻量输入框                   | 当前支持中文宽度光标、基础编辑、斜杠命令补全和菜单选择                                          |
...
│   └── darkbluff-tui/              # 渲染层（ratatui/crossterm；仅依赖 engine 门面）
│       └── src/
│           ├── lib.rs              # TUI 入口
│           ├── app/                # Session 驱动、输入分发、Outcome 消费、斜杠补全
│           ├── view/               # 布局与 widget 渲染（含 map / note / overlay）
│           ├── input.rs            # 命令输入框
│           └── terminal.rs         # raw mode / alternate screen 生命周期
├── data/                           # 正式游戏内容数据（TODO；当前仅 crates/darkbluff-core/tests/fixtures/data 可用）
│   ├── scenes/  characters/  chapters/  text/
└── docs/                           # 设计文档（本目录）
```

依赖方向由 Cargo 强制（单向、无环）：`darkbluff`（bin）→ `darkbluff-core`，`darkbluff`（bin）→ `darkbluff-tui` → `darkbluff-core`。
core 不含 clap/ratatui。`engine` 内部处理模块为私有实现细节，渲染层的游戏流程入口只使用 `darkbluff_core::engine` 暴露的 `Session` / `Input` / `Outcome` / `SessionState` / `MenuOption` 等门面类型。`content` / `save` 仍作为 core 的公共作者工具与存档 API 暴露给二进制、测试和后续工具；TUI/GUI 不应依赖这些模块来驱动游戏流程。


## 架构分层

UI 层是独立的 `darkbluff-tui` crate；引擎层 / 存档层 / 内容引擎均在 `darkbluff-core` crate 内。TUI 通过 `Session` 门面驱动游戏流程，未来 GUI 应复用同一契约。
...
- 第三方 ratatui 生态 crate（tachyonfx、tui-markdown、tui-tree-widget、tui-prompts）暂未接入。当前已用自研轻量实现覆盖 v1 所需的输入补全、基础 Markdown、map 展示和短过渡动画；后续接入前需确认它们对同一 ratatui 主版本的兼容性。**`tui-markdown` 相对小众，需先做 POC 验证**——确认其维护状态、渲染能力（中文排版、嵌套列表、代码块）是否满足需求
- `include_dir` 内嵌模式尚未实现；实现后，内嵌的 `data/` 目录变更需要重新编译
- 当前 crossterm 事件读取使用同步轮询；动画期间缩短 poll 间隔以驱动重绘。若后续接入外部异步任务，再拆到独立线程或 async task，避免阻塞渲染
- **终端最小尺寸**: 当前 TUI 要求 86×24；低于此尺寸时显示提示，不渲染主布局，避免面板错乱
...
- **退出处理**: 当前 TUI 通过 crossterm 捕获 `Ctrl+C`，Unix 下通过 `signal-hook` 捕获 `SIGTERM`。两者都走标准 `quit` 路径：保存成功后退出，保存失败则提示并留在游戏内；终端 raw mode / alternate screen 由 RAII guard 恢复
...
- **panic 恢复**: TUI 终端生命周期由 RAII guard 管理，运行期 panic 展开时会尝试恢复 raw mode、光标与 alternate screen，避免用户终端被锁死
...
- **常态事件驱动**: 当前仅在状态变脏时重绘，事件读取使用短间隔 poll 以响应输入与 resize
- **动画期间 tick 重绘**: 动画播放时启用约 33ms tick 重绘，结束后回到常态 poll
- **动画期间输入**: 过渡动画很短，玩家输入仍由同一事件循环正常处理；`motion: reduced` 缩短动画，`motion: off` / `--no-motion` 跳过动画
...
| 指令解析与补全 (`commands.rs` / `darkbluff-tui::app::suggest`) | 单测 | 各指令合法/非法输入、大小写、多空白、斜杠补全候选 |
...
- **窄终端降级**: 低于 86×24 时显示终端过小提示；`map` / `note` 面板在可用尺寸内以列表形式展示
